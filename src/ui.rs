...
fn render_output_log(frame: &mut Frame, area: Rect, state: &State) {
    let output_block = Block::new()
        ...
        .title(Span::styled(" [[[ CONSOLE LOG ]]] ", Style::new().fg(COLOR_PRIMARY).add_modifier(Modifier::BOLD)));
    
    frame.render_widget(output_block, area);

    let inner_area = area.inner(Margin { vertical: 1, horizontal: 0 });
    ...
    for (i, log) in state.command_log.iter().enumerate().rev() {
        ...
        let block_area = Rect::new(inner_area.x, inner_area.y + current_y, inner_area.width, block_height);
...
fn build_log_block(log: &CommandLog) -> Vec<Line> {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(&log.command, Style::new().fg(COLOR_FG).add_modifier(Modifier::BOLD)),
    ]));

    if !log.output.is_empty() {
        for output_line in log.output.lines() {
            let content = if let Some(stderr) = output_line.strip_prefix("[stderr] ") {
                Span::styled(stderr, Style::new().fg(COLOR_ERROR).add_modifier(Modifier::ITALIC))
            } else {
                Span::raw(output_line).fg(COLOR_FG)
            };
            lines.push(Line::from(vec![Span::styled("│  ", Style::new().fg(COLOR_COMMENT)), content]));
        }
    }
    ...
    if log.is_running {
        lines.push(Line::from(vec![
            Span::styled("│  ", Style::new().fg(COLOR_COMMENT)),
            Span::styled("⚙️  Running...", Style::new().fg(COLOR_WARN).add_modifier(Modifier::SLOW_BLINK)),
        ]));
    }
    
    lines.push(Line::from(Span::styled("╰─", Style::new().fg(COLOR_COMMENT))));
    lines.push(Line::raw(""));
...
fn render_status_bar(frame: &mut Frame, area: Rect, state: &State) {
    let status_layout = Layout::horizontal([Constraint::Percentage(50), Constraint::Percentage(50)]).split(area);
    let brand = Paragraph::new(Line::from(vec![
        Span::styled("[[[ HALO ]]]", Style::new().fg(COLOR_BG).bg(COLOR_PRIMARY).add_modifier(Modifier::BOLD)),
...
fn render_input_box(frame: &mut Frame, area: Rect, state: &State) {
    ...
    let (text, style, border_style, title_span) = if is_previewing {
        ...
        let command_text = state.command_log.get(log_index).map_or("", |log| &log.command);

        (
            Line::from(vec![Span::styled("❯  ", Style::new()), Span::styled(command_text, Style::new())]),
            Style::new().fg(COLOR_ACCENT).add_modifier(Modifier::BOLD),
            Style::new().fg(COLOR_ACCENT),
            Span::styled(" [[[ HISTORY PREVIEW ]]] ", Style::new().fg(COLOR_PRIMARY).add_modifier(Modifier::BOLD))
        )
    } else {
        (
             Line::from(vec![
                Span::styled("❯  ", Style::new().fg(COLOR_PRIMARY).add_modifier(Modifier::BOLD)),
                Span::styled(&state.input_buffer, Style::new().fg(COLOR_FG)),
            ]),
            Style::default(),
            Style::new().fg(COLOR_PRIMARY),
            Span::styled(format!("  [[[ {} ]]]  ", state.username), Style::new().fg(COLOR_ACCENT).add_modifier(Modifier::BOLD))
        )
    };
...
fn render_completion_popup(frame: &mut Frame, area: Rect, state: &mut State) {
    ...
    let items: Vec<ListItem> = suggestions.iter().map(|s| {
        let icon = if s.ends_with('/') { "📁" } else { "📄" };
        ListItem::new(Line::from(vec![Span::raw(icon), Span::raw(" "), Span::raw(s)]))
    }).collect();
    let height = (items.len() + 2).min(10) as u16;
    let popup_area = Rect { x: area.x, y: area.y.saturating_sub(height), width: area.width.min(80), height };
    ...
    .highlight_style(Style::new().bg(COLOR_PRIMARY).fg(COLOR_BG).add_modifier(Modifier::BOLD))
    .highlight_symbol("▶ ");

    let mut list_state = ListState::default().with_selected(Some(state.completion_state.selected_index));
